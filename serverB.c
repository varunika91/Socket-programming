...
#define MYPORT "22658"    // the static UDP port
...
int main() {
	...
	//Starting the TCP client code for ServerB
	...
	printf("The Server B finishes sending its neighbor information to the Client with TCP port number %d and IP address %s(Client's TCP port number and IP address).\n",ntohs(get_in_port(p->ai_addr)),s);
	...
	printf("The server B has received the network topology from the Client with UDP port number %d and IP address %s(Client's UDP port number and IP address) as follows:\n",ntohs(get_in_port((struct sockaddr *)&their_addr)),s);
